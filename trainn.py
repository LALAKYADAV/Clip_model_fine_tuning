...
Supports CASIA-WebFace (RecordIO) + DigiFace-1M (image folder),
separately or combined.

Usage examples:
    # Both datasets combined (recommended)
    python train.py --casia_root    data/casia-webface \
                    --digiface_root data/digiface

    # CASIA only
    python train.py --casia_root data/casia-webface

    # DigiFace only
    python train.py --digiface_root data/digiface

    # Track 2 — small subset
    python train.py --casia_root data/casia-webface \
                    --max_ids_casia 1000 \
                    --digiface_root data/digiface \
                    --max_ids_digiface 1000
"""
...
def train(args):
    ...
    loader, num_classes = build_dataloader(
        ...
        max_identities_casia    = args.max_ids_casia,
        max_identities_digiface = args.max_ids_digiface,
        mode                    = "train",
    )
...
if __name__ == "__main__":
    ...
    g.add_argument("--casia_root",    type=str, default=None,
                   help="Path to data/casia-webface/")
    g.add_argument("--digiface_root", type=str, default=None,
                   help="Path to data/digiface/")
    g.add_argument("--max_ids_casia",    type=int, default=None,
                   help="Cap CASIA identities (None = all 10575)")
    g.add_argument("--max_ids_digiface", type=int, default=None,
                   help="Cap DigiFace identities (None = all)")
    ...
    if args.casia_root is None and args.digiface_root is None:
        parser.error("Provide at least one of --casia_root or --digiface_root")
